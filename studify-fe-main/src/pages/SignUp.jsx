...
export default function SignUp() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      // ✅ 올바른 요청 경로
      const res = await api.post("/v1/users/signup", payload);
      const created = res?.data;

      if (created?.id) localStorage.setItem("userId", String(created.id));
      if (created?.email) localStorage.setItem("userEmail", created.email);
      if (created?.nickname) localStorage.setItem("nickname", created.nickname);
